...
2. **Liberação é desacoplada, mas há gate de FINAL com demurrage.** A *resolução* da responsabilidade Rocket × cliente (Cap. 26) é módulo plugável da Fase 11 e não bloqueia o Demurrage Core no dia a dia. **Correção (Fase 8 v1, Blueprint 20.2):** o *fechamento FINAL* de um processo **com demurrage** exige responsabilidade resolvida — enquanto `responsabilidade = EM_ANALISE`, o `FINAL` fica bloqueado (permanece `DEVOLVIDO_AGUARDANDO_TRATAMENTO`). Sem demurrage, o `FINAL` não depende de responsabilidade. Ou seja: a Fase 8 **cria e aplica** o gate de `FINAL`; a Fase 11 apenas **resolve** o `EM_ANALISE` e libera. A redação anterior ("sem gate no fechamento") fica revogada por esta seção e pela seção oficial da Fase 8.
...
**Adendo (data informada × data de recebimento).** A validação e a comparação com o tracking usam **exclusivamente a data informada no conteúdo da minuta** (`data_informada`); a data de recebimento/upload (`criado_em`) **nunca** entra no cálculo dos dias de demurrage — a minuta pode ser recebida semanas/meses após a devolução. Além disso, `data_informada ≠ tracking_return_date` **não é aceita automaticamente** só por ser cronologicamente possível: a divergência é registrada, as duas fontes são preservadas e o `effective_return_date` só é alterado após **revisão explícita** do MANAGER/ADMIN (parâmetro `aceitarDivergencia`). O primeiro `validarMinuta` sobre uma divergência retorna `divergencia_pendente` (registra o evento, não altera nada); só um segundo com a revisão explícita altera o `effective_return_date`.
...
#### Delta final aprovado (ajustes 1–8 sobre a v1.1)

1. **Lifecycle.** Refatoração aprovada: a Fase 7 **pura** permanece intocada; o `lifecycleRepository` deixa de recalcular relógios por conta própria e **consome os mesmos resultados** produzidos pelo pipeline / `RelogioRepository`.
2. **Cache `relogios`.** É a **única** projeção/cache dos relógios; a fonte de verdade continua sendo os fatos/observações do contêiner. O pipeline mantém essa tabela sincronizada (sem segundo cache divergente).
3. **Avanço do calendário (cadência interna ≠ tracking).** O avanço dos valores **não** fica atrelado só à cadência de tracking. Cria-se uma **passagem interna barata, sem chamada externa**, no máximo **uma vez por data civil**, sobre contêineres `OPEN` sem Empty Return cujo `data_final_apuracao` precisa avançar até hoje. Tracking e recálculo interno são cadências separadas.
4. **FINAL — defesa em profundidade.** Guard no orquestrador **+** proteção de banco. Ao finalizar, os `valores_apurados` ativos aplicáveis passam **OPEN → FINAL**. Processo `FINAL` não pode ter relógios/valores superseded nem `effective_return_date` materialmente alterado sem reabertura. **Exceção documental** (aprovada): `effective_return_date` NULL → `tracking_return_date` é permitida quando uma minuta posterior apenas **confirma exatamente a mesma data** já usada no fechamento (não altera valor).
5. **Seleção tarifária (correção).** **Termo por Embarque** → usar `condicoes_comerciais.tabela_id` **fixada ao embarque**; **não** selecionar nova tabela pelo 1º dia de demurrage. **Termo Único** → seleção **vigente pelo 1º dia efetivo de demurrage** do cliente, salvo instrumento comercial específico. **Exposição Rocket** → preserva a regra da Fase 4; **não** alterar nesta v1.1.
6. **Gate por `apuracaoDemurrageStatus` (aptidão monetária).** `INDETERMINADA` **bloqueia** `FINAL` obrigatoriamente. `ZERO_CONFIRMADO` pode fechar após todos devolvidos, **sem** exigir tarifa monetária. `DEMURRAGE_CONFIRMADA` exige responsabilidade resolvida **+** comprovação **+** cálculo monetário final disponível; nesta v1.1, consideram-se **aptos** os valores `ESTIMATED` ou `CONFIRMED`, e **bloqueiam** `UNAVAILABLE` e `ESTIMATED_PROVISIONAL`.
7. **Plano contraditório — corrigido.** As ocorrências antigas em "Correções de premissa" (#2) e no objetivo da Fase 11 foram reescritas: a regra oficial é a da seção da Fase 8 — `responsabilidade = EM_ANALISE` **bloqueia** `FINAL` quando há demurrage; a Fase 11 resolve e libera.
8. **Atomicidade.** O pipeline `fatos → relógios → valores → lifecycle` é **transacional**: nenhuma etapa fica commitada com as seguintes desatualizadas se algo falhar. Como `RelogioRepository.recalcular()` e `ValorApuradoRepository.registrar()` hoje abrem transação própria, serão ajustados **internamente** para aceitar um `PoolClient`/transação externa do pipeline, **preservando** os métodos atuais quando chamados isoladamente.

**Testes-gabarito (consolidados):** N→N+1 aumenta valor; minuta aumenta dias/valor; minuta reduz dias/valor; zero→demurrage cria apuração monetária; `UNAVAILABLE` mantém dias e valor indisponível (nunca zero) e **bloqueia** FINAL; `ESTIMATED_PROVISIONAL` bloqueia FINAL; `ESTIMATED`/`CONFIRMED` permitem FINAL; `INDETERMINADA` bloqueia FINAL; `ZERO_CONFIRMADO` fecha após devolução sem tarifa; mudança de `effective_return_date` supersede a versão anterior; FINAL impede recálculo (guard + banco); reabertura preserva a versão anterior e cria nova; relógios e valores usam exatamente a mesma data final; passagem de calendário roda no máx. 1×/data civil; multi-contêiner recalcula só o afetado; atomicidade (falha na etapa de valores não deixa relógios commitados). V1 intacta.
...
**Objetivo:** implementar a pré-análise de responsabilidade Rocket × cliente do Cap. 26 **como módulo plugável e opcional**, ativado somente quando o módulo Liberação expuser eventos estruturados (linha do tempo consultável). Até lá, o campo `responsabilidade` de todo contêiner com demurrage permanece `EM_ANALISE` (derivado na Fase 8). **Correção:** esse `EM_ANALISE` **bloqueia o fechamento FINAL** de processos com demurrage (Fase 8 v1, Blueprint 20.2) — não é "sem efeito bloqueante". A Fase 11 é o que **resolve** o `EM_ANALISE` (`CONFIRMADA_ROCKET`/`CONFIRMADA_CLIENTE`/`DIVIDIDA`) e assim **libera** o gate de `FINAL`. O que a Fase 11 não faz é bloquear o dia a dia operacional (monitoramento, cadência, minuta, effective_return_date, recálculo) — só o `FINAL` com demurrage depende dela.
